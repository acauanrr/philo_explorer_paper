...
const sampleNewickFiles = [
  ...
  },
  {
    ...
    src: `/datasets/newicks/news.txt`,
    datawords: [
      { word: "POLITICS", qtd: 32 },
      { word: "WELLNESS", qtd: 17 },
      { word: "ENTERTAINMENT", qtd: 16 },
      { word: "TRAVEL", qtd: 9 },
      { word: "STYLE & BEAUTY", qtd: 9 },
      { word: "PARENTING", qtd: 8 },
      { word: "HEALTHY LIVING", qtd: 6 },
      { word: "QUEER VOICES", qtd: 6 },
      { word: "FOOD & DRINK", qtd: 6 },
      { word: "BUSINESS", qtd: 5 },
    ],
  },
];
...
export default function SidebarContent(props) {
  const { setVisDataPhylo, setVisDataWords, selectedNode, handleUnifiedSelection } = usePhyloCtx();
  const {
    option,
    setOption,
    isVoronoi,
  ...
  return (
    ...
    >
      ...
      >
        {/* /////////   LAYOUT    //////// */}
        <Card size="sm" variant="outline" mx={2} mb={3}>
          <CardHeader pb={1}>
          ...
          <CardBody pt={0}>
            <RadioGroup
              ...
              color="gray.700"
            >
              <Stack direction="column" lineHeight={1.1} spacing={1}>
                <Radio value="1">
                  <Text fontSize="xs">Radial</Text>
                </Radio>
                <Radio value="2">
                  <Text fontSize="xs">Dendrogram</Text>
                </Radio>
              </Stack>
            </RadioGroup>
          </CardBody>
        ...
        <Card size="sm" variant="outline" mx={2} mb={3}>
          <CardHeader pb={1}>
            <Text fontSize="sm" fontWeight="semibold">Display Options</Text>
          </CardHeader>
          <CardBody pt={0}>
          ...
          <CardHeader pb={1}>
            <HStack fontWeight="semibold" spacing={2}>
              <Icon
                ...
                as={GiTreeBranch}
              />
              <Text fontSize="sm">Leaves</Text>
            </HStack>
          </CardHeader>
          <CardBody pt={0} spacing={3}>
            <VStack spacing={3}>
              <HStack w="full">
                <Text fontSize="xs" color="gray.700" minW="30px">
                  Size:
                </Text>
                <Slider
        ...
        <Card size="sm" variant="outline" mx={2} mb={3}>
          <CardHeader pb={1}>
            <HStack fontWeight="semibold" spacing={2}>
              ...
              />
              <Text fontSize="sm">Input Newick Format</Text>
            </HStack>
          </CardHeader>
          <CardBody pt={0}>
            <VStack spacing={3}>
              <TableContainer w="full">
                <Table variant="simple" size="2xs">
                  <Thead>
                    <Tr fontSize="xs">
                      <Th color="blue.600" fontSize="xs">Samples</Th>
                      <Th color="blue.600" fontSize="xs">Load</Th>
                    </Tr>
                  </Thead>
                  <Tbody fontSize="xs">
                    {sampleNewickFiles &&
                      sampleNewickFiles.map((f, i) => (
                        <Tr key={i} lineHeight={6}>
                          <Td>
                            <Text
                              as="a"
                              href={f.src}
                          ...
                          <Td>
                            <IconButton
                              ...
                              variant="ghost"
                              onClick={async () => {
                                if (f.fileName === "news.txt" && !f.content) {
                                  try {
                                    const response = await fetch(f.src);
          ...
          <CardHeader pb={1}>
            <HStack fontWeight="semibold" spacing={2}>
              <Icon
                ...
                as={FiInfo}
              />
              <Text fontSize="sm">Node Information</Text>
            </HStack>
          </CardHeader>
          <CardBody pt={0}>
            {selectedNode ? (
              <VStack align="start" spacing={2}>
                <Text fontSize="xs"><strong>Name:</strong> {selectedNode.data?.name || selectedNode.name}</Text>
                {selectedNode.data?.length && (
                  <Text fontSize="xs"><strong>Branch Length:</strong> {selectedNode.data.length.toFixed(6)}</Text>
                )}
                <Text fontSize="xs"><strong>Depth:</strong> {selectedNode.depth || 'Unknown'}</Text>
                {selectedNode.value > 0 && (
                  <Text fontSize="xs"><strong>Descendants:</strong> {selectedNode.value}</Text>
                )}
                <Button
                  size="xs"
                  onClick={() => {
              ...
              <Text fontSize="xs" color="gray.500" fontStyle="italic">
                Click on a node in the tree to view its details.
              </Text>
            )}
          </CardBody>
        </Card>
